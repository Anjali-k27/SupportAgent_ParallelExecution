...
Extends Session 8 with parallel agent execution via Send API.
Dispatcher fans out to specialist agents simultaneously.
Synthesizer merges all findings into one coherent response.
Sequential supervisor path preserved for single-intent tickets.
...
def route_after_triage(state: SharedState) -> str:
    ...
    Multi-issue or fraud keywords: dispatcher (parallel).
    Simple single-intent: supervisor (sequential).
    """
    ...
    if has_fraud_signals or has_multiple_issues:
        print(f"[Route:triage] Multi-issue detected → dispatcher")
        return 'dispatcher'
...
def build_master_graph():
    ...
    # All three parallel agents → synthesizer (after superstep)
    master_builder.add_edge('tech_analysis_agent',    'synthesizer')
    ...
    print("[Master Graph] Session 9 — parallel + sequential paths active")
    return graph
...
def run_session_verification() -> dict:
    """
    ┌─────────────────────────────────────────────────────────────┐
    │  SESSION 9 — VERIFICATION TEST                              │
    ├─────────────────────────────────────────────────────────────┤
    │  WHAT THIS TESTS:                                           │
    │  Dispatcher fans out correctly based on ticket content.     │
    │  All parallel agents write findings to internal_notes.      │
    │  internal_notes has all findings after superstep.           │
    │  Synthesizer reads all findings and produces response.      │
    │  Sequential supervisor path still works for simple tickets. │
    │                                                             │
    │  PASS CRITERIA:                                             │
    │  ✓ Multi-issue ticket → dispatcher fires, ≥2 findings      │
    │  ✓ No finding is overwritten — operator.add preserved all  │
    │  ✓ Synthesizer produces non-empty final_response           │
    │  ✓ Fraud ticket → fraud_analysis_agent fires               │
    │  ✓ Simple ticket still uses supervisor path correctly       │
    │                                                             │
    │  WHAT A PASS PROVES:                                        │
    │  Send API parallel execution is working.                    │
    │  Superstep boundary respected — synthesizer runs after all. │
    │  operator.add preserves all parallel findings.              │
    │  Session 10 write access can be added on top.               │
    └─────────────────────────────────────────────────────────────┘
    """
    ...
    # ── CHECK 1: Multi-issue ticket → dispatcher fires ─────────────

    thread_1 = f"verify-s9-multi-{int(time.time())}"
    result = run_ticket(
        "My API is returning 401 errors AND my account C-1002 "
        "has an unexpected charge this month.",
        thread_id=thread_1
    )
    ...
    has_multi_findings = len(agent_findings) >= 2

    check1_passed = (
        has_multi_findings and
        bool(result.get('final_response', '').strip())
    )

    checks.append({
        'label':        'Multi-issue ticket → dispatcher fires, ≥2 findings',
        'passed':       check1_passed,
        'has_response': bool(result.get('final_response', '').strip()),
        'note':         f"agent_findings={len(agent_findings)} "
                        f"agents={[n.get('agent') for n in agent_findings]}",
    })

    # ── CHECK 2: All findings in internal_notes — operator.add ─────

    finding_agents = {n.get('agent') for n in agent_findings}
    no_overwrites  = len(finding_agents) == len(agent_findings)

    check2_passed = (
        len(agent_findings) >= 2 and
        no_overwrites
    )

    checks.append({
        'label':        'All findings preserved — operator.add working',
        'passed':       check2_passed,
        'has_response': True,
        'note':         f"unique agents: {finding_agents} "
                        f"total findings: {len(agent_findings)}",
    })

    # ── CHECK 3: Synthesizer produces non-empty response ───────────
    ...
    check3_passed = has_content

    checks.append({
        'label':        'Synthesizer produces coherent unified response',
        'passed':       check3_passed,
        'has_response': has_content,
        'note':         f"response length: {len(final_response)} chars",
    })

    # ── CHECK 4: Fraud ticket → fraud_analysis_agent fires ─────────

    thread_4 = f"verify-s9-fraud-{int(time.time())}"
    result4 = run_ticket(
        "Unauthorized transactions on my account ACC-F001.",
        thread_id=thread_4
    )

    notes4 = result4.get('internal_notes', [])
    fraud_findings = [
        n for n in notes4
        if isinstance(n, dict) and n.get('agent') == 'fraud_analysis'
    ]

    check4_passed = (
        len(fraud_findings) >= 1 and
        bool(result4.get('final_response', '').strip())
    )

    checks.append({
        'label':        'Fraud ticket → fraud_analysis_agent fires',
        'passed':       check4_passed,
        'has_response': bool(result4.get('final_response', '').strip()),
        'note':         f"fraud_findings={len(fraud_findings)}",
    })

    # ── CHECK 5: Simple ticket still uses supervisor path ──────────

    thread_5 = f"verify-s9-simple-{int(time.time())}"
    result5 = run_ticket(
        "What is my subscription plan? Account C-1001.",
        thread_id=thread_5
    )

    delegation5 = result5.get('delegation_count', 0)

    check5_passed = (
        delegation5 >= 1 and
        bool(result5.get('final_response', '').strip())
    )

    checks.append({
        'label':        'Simple ticket still uses supervisor path',
        'passed':       check5_passed,
        'has_response': bool(result5.get('final_response', '').strip()),
        'note':         f"delegation_count={delegation5} "
                        f"(>0 confirms supervisor path used)",
    })
...
def run_cli_tests():
    """Runs all Session 9 test cases when file is executed directly."""

    print("\n" + "█" * 64)
    print("█  ENTERPRISE AI SUPPORT PLATFORM — SESSION 9 OF 12        █")
    print("█  Shared Scratchpads & Consensus                          █")
    print("█" * 64)

    import time as _time
    _ts = int(_time.time())

    # TEST 1 — Multi-issue ticket (parallel path)
    print(f"\n{'─' * 60}")
    print("TEST 1 — Multi-issue ticket (parallel path)")
    thread_1 = f"test-s9-multi-{_ts}"
    ticket1 = ("My API is returning 401 errors AND my account C-1002 "
               "has an unexpected charge this month.")
    print(f"TICKET:    {ticket1}")
    print(f"EXPECTED:  dispatcher fires, ≥2 agents run in parallel")
    result1 = run_ticket(ticket1, thread_id=thread_1)
    notes1 = result1.get('internal_notes', [])
    findings1 = [n for n in notes1 if isinstance(n, dict) and
                 n.get('agent') in ('tech_analysis', 'billing_analysis', 'fraud_analysis')]
    resp1  = result1.get('final_response', '')[:120]
    print(f"agent_findings:   {len(findings1)} — agents: {[n.get('agent') for n in findings1]}")
    print(f"severities:       {[n.get('severity') for n in findings1]}")
    print(f"final_response:   {resp1}")
    passed1 = len(findings1) >= 2 and bool(resp1.strip())
    print(f"Status:    {'✅ PASS' if passed1 else '❌ FAIL'}")

    # TEST 2 — Fraud signals ticket (parallel path)
    print(f"\n{'─' * 60}")
    print("TEST 2 — Fraud signals ticket (parallel path)")
    thread_2 = f"test-s9-fraud-{_ts}"
    ticket2 = ("I received alerts for transactions I did not make "
               "on account ACC-F001. Also cannot log into the dashboard.")
    print(f"TICKET:    {ticket2}")
    print(f"EXPECTED:  fraud_analysis + tech_analysis both fire")
    result2 = run_ticket(ticket2, thread_id=thread_2)
    notes2 = result2.get('internal_notes', [])
    findings2 = [n for n in notes2 if isinstance(n, dict) and
                 n.get('agent') in ('tech_analysis', 'billing_analysis', 'fraud_analysis')]
    fraud2 = [n for n in findings2 if n.get('agent') == 'fraud_analysis']
    resp2 = result2.get('final_response', '')[:120]
    print(f"agent_findings:   {len(findings2)} — agents: {[n.get('agent') for n in findings2]}")
    print(f"fraud finding:    {fraud2[0].get('severity','none') if fraud2 else 'not found'}")
    print(f"final_response:   {resp2}")
    passed2 = len(findings2) >= 1 and bool(resp2.strip())
    print(f"Status:    {'✅ PASS' if passed2 else '❌ FAIL'}")

    # TEST 3 — Simple billing ticket (supervisor path)
    print(f"\n{'─' * 60}")
    print("TEST 3 — Simple billing ticket (supervisor path)")
    thread_3 = f"test-s9-bill-{_ts}"
    ticket3 = "What is my outstanding balance? Account C-1002."
    print(f"TICKET:    {ticket3}")
    print(f"EXPECTED:  supervisor path used, delegation_count > 0")
    result3 = run_ticket(ticket3, thread_id=thread_3)
    deleg3 = result3.get('delegation_count', 0)
    nw3    = result3.get('next_worker', '')
    resp3  = result3.get('final_response', '')[:80]
    print(f"delegation_count: {deleg3}")
    print(f"next_worker:      {nw3}")
    print(f"final_response:   {resp3}")
    passed3 = deleg3 >= 1 and bool(resp3.strip())
    print(f"Status:    {'✅ PASS' if passed3 else '❌ FAIL'}")

    # TEST 4 — Simple technical ticket (supervisor path)
    print(f"\n{'─' * 60}")
    print("TEST 4 — Simple technical ticket (supervisor path)")
    thread_4 = f"test-s9-tech-{_ts}"
    ticket4 = "Getting 401 errors on API calls after SDK update."
    print(f"TICKET:    {ticket4}")
    print(f"EXPECTED:  supervisor path used")
    ...
    passed4 = bool(resp4.strip())
    print(f"Status:    {'✅ PASS' if passed4 else '❌ FAIL'}")

    # TEST 5 — Pure fraud ticket (parallel path)
    print(f"\n{'─' * 60}")
    print("TEST 5 — Pure fraud ticket (parallel path)")
    thread_5 = f"test-s9-pure-fraud-{_ts}"
    ticket5 = "Unauthorized transaction on account ACC-F001."
    print(f"TICKET:    {ticket5}")
    print(f"EXPECTED:  fraud_analysis_agent fires")
    result5 = run_ticket(ticket5, thread_id=thread_5)
    notes5 = result5.get('internal_notes', [])
    fraud5 = [n for n in notes5 if isinstance(n, dict) and n.get('agent') == 'fraud_analysis']
    resp5  = result5.get('final_response', '')[:120]
    print(f"fraud_findings:   {len(fraud5)}")
    if fraud5:
        print(f"severity:         {fraud5[0].get('severity','unknown')}")
        print(f"action:           {fraud5[0].get('action','unknown')}")
    print(f"final_response:   {resp5}")
    passed5 = len(fraud5) >= 1 and bool(resp5.strip())
    ...
    print(f"\n{'═' * 64}")
    print(f"SESSION 9 COMPLETE — {verification['summary']}")
